...
public class CheckView extends View implements Checkable {
    ...
    private static final int COLOR_DISABLED = Color.parseColor("#A8A8A8");
    private static final int COLOR_DISABLED_STROKE = Color.parseColor("#9E9E9E");
    ...
    private int mCheckedColor, mUnCheckedColor, mFloorColor, mFloorUnCheckedColor, mDisabledStrokeColor, mDisabledCheckedColor;

    private int mCheckedColorCache, mUnCheckedColorCache, mFloorColorCache, mDisabledStrokeColorCache, mDisabledCheckedColorCache;
    private boolean mChecked, mEnabled, mClickable;
    ...
    private void init(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.CheckView);
        int tickColor = ta.getColor(R.styleable.CheckView_colorTick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.CheckView_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.CheckView_colorUncheckedStroke, COLOR_FLOOR_UNCHECKED);
        mFloorColorCache = mFloorColor;
        mCheckedColor = ta.getColor(R.styleable.CheckView_colorChecked, COLOR_CHECKED);
        mCheckedColorCache = mCheckedColor;
        mUnCheckedColor = ta.getColor(R.styleable.CheckView_colorUnchecked, COLOR_UNCHECKED);
        mUnCheckedColorCache = mUnCheckedColor;

        mDisabledCheckedColor = ta.getColor(R.styleable.CheckView_colorDisabledChecked, COLOR_DISABLED);
        mDisabledCheckedColorCache = mDisabledCheckedColor;

        mDisabledStrokeColor = ta.getColor(R.styleable.CheckView_colorDisabledStroke, COLOR_DISABLED_STROKE);
        mDisabledStrokeColorCache = mDisabledStrokeColor;

        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.CheckView_strokeWidth, dp2px(getContext(), 0));
        mShape = ta.getInt(R.styleable.CheckView_shape, 0);
        mChecked = ta.getBoolean(R.styleable.CheckView_checked, false);
        mEnabled = ta.getBoolean(R.styleable.CheckView_enabled, true);
        mClickable = ta.getBoolean(R.styleable.CheckView_clickable, true);
        ta.recycle();

        //不可用时，设置为灰色显示
        if (!mEnabled) {
            mCheckedColor = mDisabledCheckedColorCache;
            mUnCheckedColor = mDisabledCheckedColorCache;
            mFloorColor = mDisabledStrokeColorCache;
        }
...
//                }
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (!mEnabled) {
            ...
            mCheckedColor = mCheckedColorCache;
            mUnCheckedColor = mUnCheckedColorCache;
            mFloorColor = mFloorColorCache;
            mFloorUnCheckedColor = mFloorColor;
        }
        reset();
        invalidate();
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        try {
            ...
            Log.e("CheckView", tr.getMessage(), tr);
        }
    }
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(CheckView.this, mChecked);
        }
    }
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(CheckView checkBox, boolean isChecked);
    }
